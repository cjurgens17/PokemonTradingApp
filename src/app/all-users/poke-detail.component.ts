...
export class PokeDetailComponent {
  src =
    'https:encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTu30k6P3xZgweDq8xtx3ZV1j9wKLqm_8ycbg&usqp=CAU';
  //pagetitle
  //Hot Observable for error messages
  private errorMessageSubject = new Subject<string>();
  errorMessage$ = this.errorMessageSubject.asObservable();

  clickedPokemon$ = this.allUsersService.clickedPokemon$.pipe(
  ...
  pageTitle$ = this.clickedPokemon$.pipe(
    map((p) => (p ? `${p.name}` : null)),
    catchError((err) => {
      this.errorMessageSubject.next(err);
      return EMPTY;
  ...
  pokeDetail$ = combineLatest([this.clickedPokemon$, this.pageTitle$]).pipe(
    filter((p) => Boolean(p)),
    map(([clickedPokemon, pageTitle]) => ({ clickedPokemon, pageTitle })),
    catchError((err) => {
      this.errorMessageSubject.next(err);
      return EMPTY;
    })
